...
def sanity_check_config() -> tuple[bool, list[str]]:
    ...
    if bridge.get("hub", {}).get("accounting_stable") != "USDT":
        issues.append("bridge hub must use USDT accounting for Celo↔Solana")
    wh = bridge.get("wormhole", {})
    if not wh.get("celo_token_bridge") or not wh.get("solana_usdt"):
    ...
    env_path = root / ".env"
    if env_path.exists() and "YOUR_CELO" in env_path.read_text():
...
def sanity_check_env() -> tuple[bool, list[str]]:
    issues: list[str] = []
    for var in ("CELO_PRIVATE_KEY", "SOLANA_SECRET_KEY", "VNX_PRIVATE_KEY_B64"):
...
def run_full_sanity() -> tuple[bool, str]:
    ...
    evidence = "; ".join(all_issues) if all_issues else "config+env ok (celo↔sol active; vnx USDC arb off)"
