...
def test_production_targets_loaded():
    ...
    assert t["celo_usdt"] == 250
    assert t["sol_usdc"] == 250


def test_funding_target_gap():
    row = FundingTarget("celo_usdt", "Celo USDT", 250, 100, "USDT")
    assert row.gap == 150
    assert not row.ok
...
def test_format_report_underfunded():
    rows = [
        ...
        FundingTarget("celo_usdt", "Celo USDT", 250, 260, "USDT"),
    ]
...
async def test_funding_report_mock():
    ...
    assert any(r.key == "celo_usdt" and not r.ok for r in rows)
