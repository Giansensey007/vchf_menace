"""Hub triangle: CELO ↔ ETH ↔ SOL composite paths."""
from unittest.mock import AsyncMock, MagicMock, patch

import pytest

from src.bridge.cctp import CctpBridgeResult
from src.bridge.hub_eth import celo_usdt_to_sol_usdc, sol_usdc_to_celo_usdt
from src.bridge.wormhole import WormholeBridgeResult


@pytest.mark.asyncio
async def test_celo_eth_sol_composite_success():
    wh_br = WormholeBridgeResult("celo_eth", 5.0, "0xwh", "0xredeem", False, True)
    cctp_br = CctpBridgeResult("eth_sol", 4.9, "0xcctp", "solsig", False, True)

    with (
        patch("src.bridge.hub_eth.wormhole_celo_to_eth", new_callable=AsyncMock) as mock_wh,
        ...
        r = await celo_usdt_to_sol_usdc(None, 5.0)
    assert r["success"]
    assert r["direction"] == "celo_usdt_to_sol_usdc"
    mock_cctp_cls.return_value.bridge_usdc_eth_to_sol.assert_awaited_once()


@pytest.mark.asyncio
async def test_sol_eth_celo_composite_success():
    wh_br = WormholeBridgeResult("eth_celo", 4.9, "0xwh2", "0xcelo", False, True)
    ...
    with (
        ...
        patch("src.bridge.hub_eth.wormhole_eth_to_celo", new_callable=AsyncMock) as mock_wh,
    ):
        ...
        r = await sol_usdc_to_celo_usdt(None, 5.0)
    assert r["success"]
    assert r["direction"] == "sol_usdc_to_celo_usdt"
