...
def _build_executive_latex() -> str:
    ...
    return "\n".join(
        [
            ...
            rf"  {{\fontsize{{17}}{{19}}\selectfont\bfseries\color{{primary}} VCHF Menace}}\\[1pt]",
            r"  {\fontsize{9}{11}\selectfont Dual-hub VCHF arbitrage — Celo (USDT) + Base (USDC) · Solana · VNX}\\[1pt]",
            rf"  {{\fontsize{{7}{8.5}}\selectfont\textcolor{{ink!60}}{{10 directed routes · {n_active}/10 active · closed-loop return after every arb}}}}",
            r"\end{minipage}&",
