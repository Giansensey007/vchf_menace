...
async def _maybe_execute(
    ...
    if br.source_tx and not br.dry_run:
        _log(f"  Base: https://celoscan.io/tx/{br.source_tx}")
    return br.success
...
async def run(amount: float, execute: bool) -> int:
    ...
    celo_addr = _CeloExec(chains["base"]).address
    ...
    if eth_probe < amount * 0.95:
        ...
        _log(
            f"ETH hot: {eth} | Base recipient: {celo_addr} | "
    ...
    _log(f"  {'PASS' if ok_eth_celo else 'FAIL'}  Ethereum → Base USDT (Wormhole)")
